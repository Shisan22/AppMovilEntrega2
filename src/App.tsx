...
function MainApp() {
  ...
  return (
    <div className="h-full w-full bg-stone-100 dark:bg-stone-950 font-sans text-stone-900 dark:text-stone-100 flex flex-col md:flex-row relative overflow-hidden transition-colors duration-300">
      ...
      <nav 
        className="absolute md:relative bottom-[calc(1.5rem+env(safe-area-inset-bottom))] md:bottom-auto left-1/2 md:left-auto -translate-x-1/2 md:translate-x-0 bg-stone-900 dark:bg-stone-800 rounded-full md:rounded-none px-2 py-2 md:py-8 flex md:flex-col items-center justify-between md:justify-center md:gap-8 shadow-2xl w-[92%] max-w-[400px] md:w-28 md:max-w-none md:h-full z-50 transition-colors duration-300"
      >
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto w-full pb-[calc(7rem+env(safe-area-inset-bottom))] md:pb-0 relative">
        {/* Global App Logo (Scrolls with content) */}
        <div className="absolute top-6 left-6 md:top-8 md:left-8 z-40 flex items-center gap-2 opacity-50 pointer-events-none select-none">
          <KoruLogo className="w-7 h-7 text-emerald-600 dark:text-emerald-500" />
          <span className="font-serif font-bold text-xl tracking-wide text-stone-800 dark:text-stone-200">Koru</span>
        </div>
